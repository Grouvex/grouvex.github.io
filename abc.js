...
import { getAuth, reauthenticateWithCredential, EmailAuthProvider, createUserWithEmailAndPassword, signInWithEmailAndPassword, updateProfile, signOut, deleteUser, onAuthStateChanged, GoogleAuthProvider, signInWithRedirect, getRedirectResult, sendEmailVerification, sendPasswordResetEmail } from "https://www.gstatic.com/firebasejs/9.6.1/firebase-auth.js";
...
function handleEmailLogin(email, password) {
    signInWithEmailAndPassword(auth, email, password)
        .then((userCredential) => {
            ...
            checkAccess(user.uid);
            alert(`Hola, ${user.displayName} (${user.email}). Disfruta de la Página Web. Si eres un miembro del equipo, puedes comentar en news aquí: https://grouvex.com/comentarios. Como usuario, puedes comentar en news aquí.`);
            redirectUser();
        })
        .catch((error) => {
...
document.addEventListener('DOMContentLoaded', function() {
    if (true){
        function mostrarnewsAdv() {
            ...
            newsAdv.innerHTML = `
                <h1 style="color: black"><span class="insignia owner-recording"></span> | Modificación de Tiempos</h1>
                <div style="margin-top: 10px; display: flex; justify-content: center; gap: 10px; flex-wrap: wrap; flex-direction: column; align-content: center;">
                    <p>El Equipo de Grouvex Studios Recording está saturado actualmente, por tanto, durante un periodo, la entrega del proyecto para lanzarlo, deberá ser <n>6 semanas antes de la fecha de venta</n>.</p>
                    <p>Entra en vigor el <n>2 de Marzo de 2025</n></p>
                    <p>Esto modifica por tanto, la sección de <n>Obligaciones del Artista > Entrega de Material</n>. <a href="https://grouvex.com/grouvex-studios-recording#Entrega_de_Material" style="color: white;">Más información</a></p>
        ...
        onAuthStateChanged(auth, (user) => {
            ...
            if (mantenimientoActivo) {
                if (paginaActual !== paginaMantenimiento && !(user && uidsTeam.includes(user.uid))) {
                    alert(`🚧 La Página está en mantenimiento, se te redirigirá a ${paginaMantenimiento}.`);
